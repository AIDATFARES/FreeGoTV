...
export default function ResellerPageContent() {
  ...
  return (
    <section className="relative overflow-hidden bg-white py-16 sm:py-20 lg:py-24 text-format-marketing">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[800px] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-red-950/25 via-white to-transparent" />

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        
        {/* HERO SECTION */}
        <header className="relative mx-auto max-w-4xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-[#7D927D]/30 bg-[#7D927D]/10 px-4 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.18em] text-[#7D927D]">
            <Sparkles className="h-3.5 w-3.5" />
            IPTV Reseller Program
          </span>

          <h1 className="mt-6 text-4xl sm:text-5xl lg:text-6xl font-black leading-[1.05] tracking-tight text-[#2F3136]">
            <span className="block text-[#2F3136]">Become an IPTV Reseller</span>
          ...
          <div className="mt-8 flex items-center justify-center gap-4">
            <button
              ...
              className="rounded-full bg-[#FF6B00] px-8 py-3.5 text-base font-bold text-[#2F3136] shadow-[0_0_25px_rgba(255,107,0,0.4)] transition-all duration-300 hover:bg-red-700 hover:shadow-[0_0_35px_rgba(255,107,0,0.6)] active:scale-[0.98]"
            >
              Free Trial
            </button>
          </div>
        ...
        <div className="mt-24">
          <div className="text-center">
            <h2 className="text-3xl sm:text-4xl font-black leading-tight tracking-tight">
              <span className="block text-[#2F3136]">Our Exclusive</span>
              ...
              <article
                ...
                className={`relative flex flex-col rounded-3xl bg-white p-6 shadow-2xl transition-all duration-300 hover:-translate-y-2 ${
                  plan.popular
                    ...
                    : plan.bestValue
                    ? "border-4 border-[#7D927D] shadow-[0_10px_40px_rgba(16,185,129,0.2)] z-10"
                    : "border-2 border-stone-100"
                }`}
              >
                {plan.popular && (
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-[#FF6B00] px-4 py-1 text-[11px] font-extrabold uppercase tracking-wider text-[#2F3136] shadow-md">
                    Most Popular
                  </span>
                )}
                {plan.bestValue && (
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 rounded-full bg-[#7D927D] px-4 py-1 text-[11px] font-extrabold uppercase tracking-wider text-[#2F3136] shadow-md">
                    Best Value
                  </span>
                )}
                ...
                <div className="text-center">
                  ...
                  <h3 className="mt-1 text-2xl font-black text-[#2F3136]">
                    {plan.credits} Credit
                  </h3>
                </div>
                ...
                <ul className="flex-grow space-y-3 border-t border-stone-100 pt-4 text-xs font-semibold text-[#68717A]">
                  {planFeatures.map((feature) => (
                    <li className="flex items-center gap-2" key={feature}>
                      <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-red-100 text-[#FF6B00]">
                ...
                <button
                  ...
                  className="mt-6 w-full rounded-2xl bg-[#FF6B00] py-3.5 text-sm font-extrabold text-[#2F3136] shadow-[0_4px_15px_rgba(255,107,0,0.3)] transition-all duration-300 hover:bg-red-700 hover:shadow-[0_6px_20px_rgba(255,107,0,0.4)] active:scale-[0.98]"
                >
                  Get A Panel
                </button>
              </article>
        ...
        <div className="mt-28">
          <h2 className="text-center text-3xl sm:text-4xl font-black leading-tight tracking-tight">
            <span className="block text-[#2F3136]">Advantages of</span>
          ...
          <div className="mt-12 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
            {advantages.map((item) => {
              ...
              return (
                <div
                  key={item.title}
                  className="flex flex-col items-center text-center p-6 rounded-2xl border border-black/10 bg-white/90 backdrop-blur-md shadow-xl hover:border-[#7D927D]/40 transition-colors"
                >
                  <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-[#7D927D]/15 text-[#7D927D] mb-5">
                    <Icon className="h-8 w-8" />
                  </div>
                  <h3 className="text-sm font-extrabold uppercase tracking-wider text-[#2F3136]">
                    {item.title}
                  </h3>
                  <p className="mt-3 text-xs leading-5 text-[#68717A]">
                    {item.description}
                  </p>
                </div>
              );
        ...
        <div className="mx-auto mt-28 max-w-4xl">
          <h2 className="text-center text-3xl sm:text-4xl font-black leading-tight tracking-tight">
            <span className="block text-[#2F3136]">Frequently Asked</span>
          ...
          <div className="mt-10 space-y-3">
            {resellerFaqs.map((faq, index) => {
              ...
              return (
                <div
                  key={faq.question}
                  className="overflow-hidden rounded-xl border border-black/10 bg-white/90 shadow-md transition-colors hover:border-[#7D927D]/30"
                >
                  <button
                    onClick={() => toggleFaq(index)}
                    className="flex w-full items-center justify-between px-6 py-4 text-left font-bold text-[#2F3136] transition-colors hover:bg-white/[0.03]"
                  >
                    <span className="text-sm sm:text-base tracking-wide uppercase font-extrabold flex items-center gap-3">
                      <span className="text-[#7D927D]">▶</span>
                      {faq.question}
                    </span>
                    {isOpen ? (
                      <ChevronUp className="h-5 w-5 shrink-0 text-[#7D927D]" />
                    ) : (
                      <ChevronDown className="h-5 w-5 shrink-0 text-[#68717A]" />
                    )}
                  </button>
          ...
          <div className="mt-12 flex justify-center">
            <button
              ...
              className="rounded-full bg-[#7D927D] px-10 py-4 text-base font-extrabold uppercase tracking-wider text-[#2F3136] shadow-[0_0_30px_rgba(255,107,0,0.5)] transition-all duration-300 hover:bg-[#7D927D] hover:shadow-[0_0_40px_rgba(255,107,0,0.7)] active:scale-[0.98]"
            >
              Become Reseller
            </button>
          </div>
        ...
        {/* TRUST BADGE */}
        <div className="mt-16 flex items-center justify-center gap-2 text-xs font-semibold text-[#68717A]">
          <ShieldCheck className="h-4 w-4 text-[#7D927D]" />
