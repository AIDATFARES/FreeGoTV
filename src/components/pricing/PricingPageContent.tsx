...
const billingQuestions = [
  { question: "1. Which payment methods can I use?", answer: <>Available payment options are shown when you place your order. <Link className="font-semibold text-[#7D927D] hover:text-[#7D927D] transition-colors" href="/contact">Contact FreeGoTV support</Link> if you need help before purchasing.</> },
  ...
  { question: "3. Will my subscription renew automatically?", answer: <>Renewal details are provided when you order. If you have any questions about your subscription period or renewal, <Link className="font-semibold text-[#7D927D] hover:text-[#7D927D] transition-colors" href="/contact">contact support</Link> before your plan expires.</> },
  { question: "4. Can I change my plan or number of connections?", answer: <>Yes. <Link className="font-semibold text-[#7D927D] hover:text-[#7D927D] transition-colors" href="/contact">Contact the support team</Link> with your order email and the plan or connection change you need, and they will advise on the available options.</> },
];
    ...
    <section className="relative overflow-hidden bg-[#f8fafc] py-16 sm:py-20 lg:py-24">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <header className="mx-auto max-w-3xl text-center mb-16">
          <span className="inline-flex rounded-full border border-[#7D927D]/30 bg-[#7D927D]/10 px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.18em] text-[#7D927D]">FreeGoTV</span>
          <h1 className="mt-6 text-4xl sm:text-5xl lg:text-6xl font-black leading-[1.05] tracking-tight">
            <span className="block text-[#2F3136] uppercase">Choose Your Exclusive</span>
            <span className="mt-1 block text-[#7D927D] uppercase">Subscription Plans.</span>
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-sm leading-6 text-[#68717A] sm:text-base">Choose the FreeGoTV plan that fits you, with secure checkout and helpful 24/7 support.</p>
          <p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-[#68717A]">Need help getting started? Visit our <Link className="font-semibold text-[#7D927D] hover:text-[#7D927D]" href="/installation">installation guide</Link> or browse the <Link className="font-semibold text-[#7D927D] hover:text-[#7D927D]" href="/channels">channel list</Link> before you order.</p>
        </header>
        ...
        <div className="mx-auto mt-16 max-w-4xl">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 rounded-2xl bg-white p-6 shadow-sm border border-stone-100 border-l-4 border-l-orange-500">
            <div>
              <h4 className="text-lg font-bold text-[#2F3136]">Secure Payments & Instant Access</h4>
              <p className="mt-1 text-sm text-[#68717A] font-medium">Pay safely using Crypto, Credit Card, or PayPal. Your details are encrypted instantly.</p>
            </div>
            <div className="flex shrink-0 items-center gap-4 text-[#68717A]">
        ...
        <section className="mt-24">
          <h2 className="text-center text-3xl font-bold text-[#2F3136] uppercase">Every plan includes</h2>
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {includedFeatures.map(({ icon: Icon, title, text }) => (
              <article className="rounded-2xl border border-stone-100 bg-white p-6 text-center shadow-sm" key={title}>
                <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-red-50 text-[#7D927D]">
                  <Icon className="h-6 w-6" />
                </div>
                <h3 className="mt-5 text-lg font-bold text-[#2F3136]">{title}</h3>
                <p className="mt-3 text-sm leading-6 text-[#68717A]">{text}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="mx-auto mt-24 max-w-4xl">
          <h2 className="text-center text-3xl font-black text-[#2F3136] uppercase">Billing &amp; Subscription FAQ</h2>
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-5">
            {billingQuestions.map((item) => (
              <div className="flex flex-col rounded-xl overflow-hidden border border-stone-200 bg-white shadow-sm hover:border-red-200 transition-colors" key={item.question}>
                <div className="bg-stone-50 border-b border-stone-100 px-5 py-4 flex items-start gap-3">
                  <HelpCircle className="w-5 h-5 text-[#7D927D] shrink-0 mt-0.5" />
                  <h3 className="text-base sm:text-lg font-bold text-[#2F3136] leading-snug">
                    {item.question}
                  </h3>
                </div>
                <div className="p-5 text-sm text-[#68717A] leading-relaxed bg-white flex-1">
                  <p>{item.answer}</p>
                </div>
              </div>
            ))}
        ...
        <div className="mt-16 flex items-center justify-center gap-2 text-sm text-[#68717A]">
          <ShieldCheck className="h-5 w-5 text-[#7D927D]" />
          <span className="font-semibold text-[#2F3136]">Secure ordering</span> and friendly FreeGoTV support.
        </div>
      </div>
    </section>
  );
