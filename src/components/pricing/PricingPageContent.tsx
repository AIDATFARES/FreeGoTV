...
const billingQuestions = [
  { question: "1. Which payment methods can I use?", answer: <>Available payment options are shown when you place your order. <Link className="font-semibold text-[#0EA5E9] hover:text-[#0EA5E9] transition-colors" href="/contact">Contact FreeGoTV support</Link> if you need help before purchasing.</> },
  ...
  { question: "3. Will my subscription renew automatically?", answer: <>Renewal details are provided when you order. If you have any questions about your subscription period or renewal, <Link className="font-semibold text-[#0EA5E9] hover:text-[#0EA5E9] transition-colors" href="/contact">contact support</Link> before your plan expires.</> },
  { question: "4. Can I change my plan or number of connections?", answer: <>Yes. <Link className="font-semibold text-[#0EA5E9] hover:text-[#0EA5E9] transition-colors" href="/contact">Contact the support team</Link> with your order email and the plan or connection change you need, and they will advise on the available options.</> },
];
    ...
    <section className="relative overflow-hidden bg-[#f8fafc] py-16 sm:py-20 lg:py-24">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <header className="mx-auto max-w-3xl text-center mb-16">
          <span className="inline-flex rounded-full border border-[#0EA5E9]/30 bg-[#0EA5E9]/10 px-4 py-1.5 text-[10px] font-extrabold uppercase tracking-[0.18em] text-[#0EA5E9]">FreeGoTV</span>
          <h1 className="mt-6 text-4xl sm:text-5xl lg:text-6xl font-black leading-[1.05] tracking-tight">
            <span className="block text-[#0F172A] uppercase">Premium IPTV Subscription</span>
            <span className="mt-1 block text-[#0EA5E9] uppercase">Plans &amp; Pricing</span>
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-sm leading-6 text-[#64748B] sm:text-base">Choose the FreeGoTV plan that fits you, with secure checkout and helpful 24/7 support.</p>
          <p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-[#64748B]">Need help getting started? Visit our <Link className="font-semibold text-[#0EA5E9] hover:text-[#0EA5E9]" href="/installation">installation guide</Link> or browse the <Link className="font-semibold text-[#0EA5E9] hover:text-[#0EA5E9]" href="/channels">channel list</Link> before you order.</p>
        </header>
        ...
        <div className="mx-auto mb-10 max-w-2xl" role="radiogroup" aria-label="Number of device connections">
          ...
          <div className="grid grid-cols-3 rounded-2xl border border-[#0EA5E9]/30 bg-white p-1.5 shadow-sm">
            {[1, 2, 3].map((count) => {
              const selected = devices === count;
              return (
                <button
                  aria-checked={selected}
                  className={`rounded-xl px-3 py-3 text-sm font-bold transition-all ${selected ? "bg-[#14B8A6] text-[#0F172A] shadow-lg shadow-sky-500/25" : "text-[#64748B] hover:text-[#0F172A] hover:bg-sky-50"}`}
                  key={count}
          ...
          {plans.map((plan) => (
            <article 
              ...
              className={`relative flex flex-col rounded-2xl bg-white p-8 transition-transform duration-300 hover:-translate-y-1 border-2 ${
                plan.popular 
                  ? "border-[#0EA5E9] shadow-[0_15px_40px_-10px_rgba(14,165,233,0.25)] md:-mt-4" 
                  : "border-sky-300 shadow-sm hover:shadow-md hover:border-[#0EA5E9]"
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 flex items-center justify-center rounded-b-xl rounded-t-sm bg-gradient-to-r from-[#14B8A6] to-slate-800 px-5 py-1.5 shadow-md whitespace-nowrap">
              ...
              <div className="mt-6 flex items-baseline">
                <span className="text-5xl font-black tracking-tighter text-[#0EA5E9]">${priceFor(plan)}</span>
              ...
              <ul className="mt-6 flex-grow space-y-4">
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-3">
                    {feature.included ? (
                      <CheckCircle2 className={`h-5 w-5 shrink-0 ${plan.popular ? "text-[#0EA5E9]" : "text-blue-500"}`} strokeWidth={2.5} />
                    ) : (
              ...
              <button 
                ...
                className={`mt-8 w-full rounded-xl py-4 text-sm font-bold text-[#0F172A] transition-all duration-300 ${
                  plan.popular 
                    ? "bg-[#14B8A6] shadow-[0_8px_20px_rgba(14,165,233,0.3)] hover:bg-[#0C8CE9] hover:-translate-y-0.5" 
                    : "bg-[#0F172A] hover:bg-[#14B8A6] text-white hover:shadow-lg hover:-translate-y-0.5"
                }`}
              >
                {plan.buttonText}
              </button>
          ...
          <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {includedFeatures.map(({ icon: Icon, title, text }) => (
              <article className="rounded-2xl border border-stone-100 bg-white p-6 text-center shadow-sm" key={title}>
                <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-sky-50 text-[#0EA5E9]">
        ...
        <div className="mt-16 flex items-center justify-center gap-2 text-sm text-[#64748B]">
          ...
          <span className="font-semibold text-[#0F172A]">Secure ordering</span> and friendly FreeGoTV support.
        </div>
      </div>
    </section>
  );
